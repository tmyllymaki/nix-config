{
  flake.hjemModules.easyeffects = {
    config,
    lib,
    pkgs,
    ...
  }: {
    options.custom.home.easyeffects.enable = lib.mkEnableOption "home.easyeffects";

    config = lib.mkIf config.custom.home.easyeffects.enable {
      packages = [
        pkgs.easyeffects
        pkgs.rnnoise
        pkgs.rnnoise-plugin
      ...
      files.".local/share/easyeffects/input/noise-reduction.json".text = ''
        {
          "input": {
            "blocklist": [],
            "compressor#0": {
